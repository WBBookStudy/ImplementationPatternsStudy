...
### 메소드 
객체 생성과 더불어 메소드 역시 프레임워크의 사용과 수정에 영향을 미친다. 프레임워크 메소드를 만들 때 전략은 생성과 크게 다르지 않다.  
클라이언트가 사용할 수 있는 취득 메소드나 설정 메소드는 자료 구조를 수정하지 않는 경우에만 사용할 수 있다. 클라이언트 내부가 자료 구조에 의존하도록 하면 프레임워크를 수정하는 것은 무척 어려워진다. 이런 점에서 설정 메소드느느 취득 메소드보다 더 나쁘다.  
떄로는 다른 방법을 사용해서 필드에 저장할 값을 구하는 것도 가능하다. 설정 메소드를 공개하는 것보다는 클라이언트가 해결하려는 문제를 메소드 이름에 반영하는 것이 좋다.  
```
ex)
위젯 라이브러리 Widget에서 setVisible(Bool)을 제공한다고 하자.
제3의 상태인 비활성화 상태가 필요한 경우는 어떻게 해야할까?
클라이언트 입장에서 간단한 사용이 가능하도록 visible()과 invisible()메소드를 제공하면 기존 setVisible(Bool)메소드의 역할을 대신할 수 있다.
이 상태에서 inactive()메소드를 상위 클래스에 추가하면 클라이언트 코드를 수정하지 않고서도 원하는 바를 이룰 수 있다.

인터페이스에 기반한 추상화는 조금 다름
inactive() 메소드를 인터페이스에 추가하면 기존 Widget에 대한 클라이언트의 구현이 동작하지 않게 된다. 
대신 상태를 나타내는 Enum타입인 States를 사용해서 setVisible(State)메소드를 제공해주자.
```
-> 설정 메소드나 취득 메소드를 절대 공개하면 안될까? 아님. 중요한 프레임워크 기능이 필드를 반환하거나 설정에서 구현된다면 접근자를 공개해도 괜찮다. 다만 이름에 내부 구현 대신 역할을 반영하자.  
  
이미 공개된 메소드에 파라미터를 추가할 경우에는 호환성을 유지하기 위해 기본값을 제공하라.  
```Java
...
```
이 경우 run(Class...)를 호출했던 모든 클라이언트는 TestResult 파라미터를 추가하기 위해 코드를 바꿔야 한다.  
이럴땐 아래와 같이 기존메소드에서 기본 파라미터를 제공해주게 할 수도 있다.
```Java
public TestResult run(Class... classes) {
...
```
> 기본 파라미터를 제공하면 인터페이스에 새로운 메소드가 추가되었음에도 클라이언트 코드는 계속해서 동작하게 된다.  
